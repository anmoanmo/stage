...
void test_logger()
{
    mylog::Logger::ptr logger = mylog::LoggerManager::getInstance().getLogger("async_log");
    ...
    // while (count < 500000)
    // {
    //     logger->info("%s+%zu", msg.c_str(), ++count);
...
int main()
{
    ...
    builder->buildLoggerSink<RollBySizeSink>("./logfile/test_roll__async", 10000);
    builder->buildAsyncBufferMax(1024 * 1024 * 1024);
